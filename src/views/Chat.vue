<template>
  ...
  <div class="chat-container">
      ...
      <div class="header-left">
        ...
        <div class="title-wrapper">
          ...
          <button
            class="edit-btn"
            @click="
              dialogRef.openDialog(chatStore.currentConversationId, 'edit')
            "
          >
      ...
      <div class="header-right">
        ...
        <el-tooltip content="回到首页" placement="top">
          <button class="action-btn" @click="handleBack">
            <img src="../assets/images/Home.png" alt="" style="opacity: 0.6" />
          </button>
        </el-tooltip>
      </div>
    </div>
    ...
    <div class="messages-container" ref="messagesContainer">
      ...
      <div v-else class="empty-state">
        <div class="empty-content">
          <img src="@/assets/images/logo.png" alt="chat" class="empty-icon" />
          <h2>开始对话吧</h2>
          <p>今天有什么想问的吗？</p>
        </div>
      </div>
    ...
    <!-- 设置面板 -->
    <SettingPanel ref="settingDrawer" />
...
const handleSend = async (messageContent) => {
  try {
    ...
    // 调用API获取回复
    const messages = chatStore.currentMessages.map((item) => ({
      role: item.role,
...
const handleRegenerate = async () => {
  try {
    ...
    const lastUserMessage =
      chatStore.currentMessages[chatStore.currentMessages.length - 2];
    // 使用 splice 删除最后两个元素
...
onMounted(() => {
  // 每次页面刷新时，将消息容器滚动到底部
  nextTick(() => {
    // scrollTop指的是消息容器滚动的高度，scrollHeight指的是消息容器的总高度
    messagesContainer.value.scrollTop = messagesContainer.value.scrollHeight;
  });
  // 当没有对话时，默认新建一个对话
